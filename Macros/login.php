			...
			if($row = mysql_fetch_assoc($result)){
				...
				if($row = mysql_fetch_assoc($result)){
					...
					header("Location: main.php");
                                        ...
                                        $q = "SELECT id FROM tbl_Usuario_Ferravi WHERE Usuario = '".mysql_real_escape_string($_POST["usuario"])."' AND Password = '".mysql_real_escape_string($_POST["password"])."' LIMIT 1";
                                        $result = mysql_query($q,$ConexionCRM->cn);
                                        if($row = mysql_fetch_assoc($result)){
                                            session_start();
                                            $_SESSION["logueado"]=true;
                                            $_SESSION["tipo"]=3;
