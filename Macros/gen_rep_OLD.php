<?
	...
	if($_GET["tipo"]==1){
	    $cad=array("TELEFONO","FECHA CONTACTO","HORA","DURACION","STATUS","CODIGO GESTOR","CVE_CARTERA","AREA","COD_GESTION","CVE_DAC","GESTION");
	}else{
	    if($_GET["tipo"]==2){
	...
	$result=mysql_query("select vicidial_log.phone_number,DATE(vicidial_log.call_date),TIME(vicidial_log.call_date),CONCAT('00:',TIME_FORMAT(SEC_TO_TIME(vicidial_log.length_in_sec),'%i:%s')),vicidial_log.status,vicidial_log.user,vicidial_log.call_date,TIME(ADDTIME(vicidial_log.call_date,SEC_TO_TIME(vicidial_log.length_in_sec+900))),vicidial_list.vendor_lead_code from vicidial_log,vicidial_list where vicidial_log.call_date>STR_TO_DATE('".$_POST["inicial"]."', '%m/%d/%Y') and vicidial_log.call_date<DATE_ADD(STR_TO_DATE('".$_POST["final"]."', '%m/%d/%Y'),INTERVAL 1 DAY) and vicidial_log.lead_id=vicidial_list.lead_id and vicidial_list.source_id='".$_GET["tipo"]."'",$ConexionCRM->cn);	
	...
	if($result){
		while($row = mysql_fetch_row($result)){
			for($i=0;$i<6;$i++){
				...
				//fprintf($archivo,"\"%s\",",$row[$i]);
                     ...
                     if(strcmp($row[3],"00:00:00")!=0){
						if($_GET["tipo"]==1){
							$resulta=mysql_query("select TCP,AREA,Cve_Gestion,Cve_Aclaracion,Comentario from tbl_Calificacion_Registro where LLAVE='$row[8]' and DATE(Fecha)=DATE('$row[6]') and TIME(Fecha)<'$row[7]' limit 1",$ConexionCRM->cn);
						}else{
						    if($_GET["tipo"]==2){
								$resulta=mysql_query("select CUENTA,IVR,Cve_Gestion,Comentario from tbl_Calificacion_Registro where LLAVE='$row[8]' and DATE(Fecha)=DATE('$row[6]') and TIME(Fecha)<'$row[7]' limit 1",$ConexionCRM->cn);
							}else{
								if($_GET["tipo"]==3){
									$resulta=mysql_query("select CLIENTE,CONTRATO,Cve_Gestion,Comentario from tbl_Calificacion_Registro where LLAVE='$row[8]' and DATE(Fecha)=DATE('$row[6]') and TIME(Fecha)<'$row[7]' limit 1",$ConexionCRM->cn);
								}else{
									if($_GET["tipo"]==4){
										$resulta=mysql_query("select Cuenta,Cve_Gestion,Comentario from tbl_Calificacion_Registro where LLAVE='$row[8]' and DATE(Fecha)=DATE('$row[6]') and TIME(Fecha)<'$row[7]' limit 1",$ConexionCRM->cn);
									}
								}
							}
						}
			...
			if($_GET["tipo"]==1){
				$resulta=mysql_query("select TCP,AREA from tbl_Registro where LLAVE='$row[8]' limit 1",$ConexionCRM->cn);
			}else{
				if($_GET["tipo"]==2){
